...
let context = function() {
  ...
  // grab team data dot json using fs read and JSON parse.
  // use nba.teamIdFromName(name) to get the team id from the name to match up
  //  source and destination for information
  // we can now add whatever team infor we want to that dict, and have access to
  //  it anywhere in our stack!
  ...
  that.updateGames = function(gameData) {
    // TODO - these shouldn't be creating new dicts and assigning everything.
    // instead, we want to set each attribute using an equals if its not already
    // there. that gives us an opportunity to check for the changes!
