  ...
  // the stats for that players season to date
  {
    ...
    response: function(channel, matched, context) {
      ...
      nbaRequest.getPlayerStatsV2(playerName, function(err, data) {
        ...
        if (data) {
          // the keys we're going to display
          let keys = {
            fg: ['fgm', 'fga', 'fgPct'],
            fg3: ['fG3M', 'fG3A', 'fg3Pct'],
            ft: ['ftm', 'fta', 'ftPct'],
            reb: ['oreb', 'dreb', 'reb'],
            rest: ['ast', 'stl', 'blk', 'tov', 'pf', 'pts']
          };

          // start the response with the players full name
          let response = data.player.fullName + '\n';

          // loop through the keys we care about, adding them to the response
          for (let set in keys) {
            response += "\t";
            for (let key in keys[set]) {
              let id = keys[set][key];
              response += data.stats[id] + ' ' + id;
              if (key < keys[set].length - 1) {
                response += ' | ';
              }
            }
            response += "\n";
