...
let getSalary = function(playerName) {
    let players = require('../data/salaries.json');
...
let getPlayerStats = function (player) {
    let requestInfo = { PlayerID: player.playerId };
    return nba.stats.playerSplits(requestInfo);
};

let getPlayersStats = function (playerName) {
...
let getStandings = function (args) {
  ...
  let addToStandings = (teamInfo) => {
    if (!teamInfo) return;

    // derp - thanks for the formatting nba.com
    let info = teamInfo.teamInfoCommon[0];
